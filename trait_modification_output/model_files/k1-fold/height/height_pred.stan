...
generated quantities {
	...
	{
		vector[N] f_ovuleArea_rep;
		vector[N] f_ovuleACov_rep;

		f_ovuleArea_rep = gp_pred_rng(X_ovuleArea, ovuleArea, X_ovuleArea,
		...
		for (n in 1:N) {
			ovuleArea_rep[n] = normal_rng(f_ovuleArea_rep[n], Sigma_ovuleArea);
...

	{
		...
		for (n in 1:NPred) {
			...
			X_gynLenPred[n, 3] = timeToFlower_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_numPodsMainPred[n, 3] = percAbortedMain_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_podLenPred[n, 1] = ovuleArea_4pred[n]; //parents ordered alphabetically
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_percAborted2aryPred[n, 1] = height_4pred[n]; //parents ordered alphabetically
			X_percAborted2aryPred[n, 2] = numFlowers_pred[n]; //parents ordered alphabetically
			X_percAborted2aryPred[n, 3] = percAbortedMain_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_numPods2aryPred[n, 2] = percAborted2ary_pred[n]; //parents ordered alphabetically
			X_numPods2aryPred[n, 3] = percAbortedMain_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_timeToMaturePred[n, 2] = num2aryBranch_4pred[n]; //parents ordered alphabetically
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_tenPodAreaPred[n, 1] = podLen_pred[n]; //parents ordered alphabetically
			X_tenPodAreaPred[n, 2] = styleLen_4pred[n]; //parents ordered alphabetically
			X_tenPodAreaPred[n, 3] = tenPodNumber_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_totSeedNumPred[n, 3] = timeToFlower_pred[n]; //parents ordered alphabetically
			X_totSeedNumPred[n, 4] = totSeedArea_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_totCompactnessPred[n, 2] = ovaryLen_pred[n]; //parents ordered alphabetically
			X_totCompactnessPred[n, 3] = seedACov_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_totSeedWPred[n, 1] = gynLen_pred[n]; //parents ordered alphabetically
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_oilContentPred[n, 1] = ovuleACov_4pred[n]; //parents ordered alphabetically
			X_oilContentPred[n, 2] = ovuleNum_pred[n]; //parents ordered alphabetically
			X_oilContentPred[n, 3] = seedACov_pred[n]; //parents ordered alphabetically
			X_oilContentPred[n, 4] = totCompactness_pred[n]; //parents ordered alphabetically
			X_oilContentPred[n, 5] = totSeedW_pred[n]; //parents ordered alphabetically
		}
