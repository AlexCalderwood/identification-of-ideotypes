...
parameters {
	...
	vector<lower=0>[D_styleLen] Rho_styleLen; //for automatic relevance determination, each parent gets own Rho
...
model {
	...
	matrix[N, N] K_styleLen = rbf(X_styleLen, X_styleLen, Alpha_styleLen, Rho_styleLen)
			 + diag_matrix(rep_vector(square(Sigma_styleLen), N));
	matrix[N, N] K_L_styleLen = cholesky_decompose(K_styleLen);
	...
	Alpha_styleLen ~ normal(0,1);
	Rho_styleLen ~ inv_gamma(5, 5);
	Sigma_styleLen ~ normal(0, 1);
	...
	styleLen ~ multi_normal_cholesky(rep_vector(0,N), K_L_styleLen);
