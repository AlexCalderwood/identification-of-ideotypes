...
data{
	...
	vector<lower=0>[D_styleLen] Rho_styleLen;
	real<lower=0> Alpha_styleLen;
	...
	vector[N] styleLen;
	vector[D_styleLen] X_styleLen[N];
	vector[N] numFlowers;
	vector[N] ovuleNum;
	vector[N] gynLen;
	vector[N] ovuleArea;
...
generated quantities {
	...
	{
		vector[N] f_styleLen_rep;

		f_styleLen_rep = gp_pred_rng(X_styleLen, styleLen, X_styleLen,
					Alpha_styleLen, Rho_styleLen, Sigma_styleLen, delta);

		for (n in 1:N) {
			styleLen_rep[n] = normal_rng(f_styleLen_rep[n], Sigma_styleLen);
		}
	}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_numFlowersPred[n, 1] = num2aryBranch_4pred[n]; //parents ordered alphabetically
			X_numFlowersPred[n, 2] = timeToFlower_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_ovuleNumPred[n, 1] = ovaryLen_pred[n]; //parents ordered alphabetically
			X_ovuleNumPred[n, 2] = timeToFlower_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_gynLenPred[n, 1] = ovaryLen_pred[n]; //parents ordered alphabetically
			X_gynLenPred[n, 2] = styleLen_4pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_ovuleAreaPred[n, 1] = numFlowers_pred[n]; //parents ordered alphabetically
			X_ovuleAreaPred[n, 2] = styleLen_4pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_numPodsMainPred[n, 1] = height_4pred[n]; //parents ordered alphabetically
			X_numPodsMainPred[n, 2] = num2aryBranch_4pred[n]; //parents ordered alphabetically
			X_numPodsMainPred[n, 3] = numFlowers_pred[n]; //parents ordered alphabetically
			X_numPodsMainPred[n, 4] = ovuleACov_4pred[n]; //parents ordered alphabetically
			X_numPodsMainPred[n, 5] = percAbortedMain_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_podLenPred[n, 2] = percAbortedMain_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_beakLenPred[n, 1] = height_4pred[n]; //parents ordered alphabetically
			X_beakLenPred[n, 2] = numPodsMain_pred[n]; //parents ordered alphabetically
			X_beakLenPred[n, 3] = podLen_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_numPods2aryPred[n, 1] = numFlowers_pred[n]; //parents ordered alphabetically
			X_numPods2aryPred[n, 2] = ovuleArea_pred[n]; //parents ordered alphabetically
			X_numPods2aryPred[n, 3] = percAborted2ary_pred[n]; //parents ordered alphabetically
			X_numPods2aryPred[n, 4] = percAbortedMain_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_timeToMaturePred[n, 1] = beakLen_pred[n]; //parents ordered alphabetically
			X_timeToMaturePred[n, 2] = ovuleACov_4pred[n]; //parents ordered alphabetically
			X_timeToMaturePred[n, 3] = percAborted2ary_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_tenPodNumberPred[n, 3] = ovaryLen_pred[n]; //parents ordered alphabetically
			X_tenPodNumberPred[n, 4] = ovuleNum_pred[n]; //parents ordered alphabetically
			X_tenPodNumberPred[n, 5] = podLen_pred[n]; //parents ordered alphabetically
			X_tenPodNumberPred[n, 6] = timeToFlower_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_tenPodAreaPred[n, 1] = styleLen_4pred[n]; //parents ordered alphabetically
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_totSeedAreaPred[n, 1] = numPods2ary_pred[n]; //parents ordered alphabetically
			X_totSeedAreaPred[n, 2] = tenPodArea_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_totSeedNumPred[n, 3] = tenPodNumber_pred[n]; //parents ordered alphabetically
			X_totSeedNumPred[n, 4] = timeToFlower_pred[n]; //parents ordered alphabetically
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_seedACovPred[n, 1] = beakLen_pred[n]; //parents ordered alphabetically
			X_seedACovPred[n, 2] = totSeedNum_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_totCompactnessPred[n, 1] = gynLen_pred[n]; //parents ordered alphabetically
			X_totCompactnessPred[n, 2] = seedACov_pred[n]; //parents ordered alphabetically
			X_totCompactnessPred[n, 3] = timeToFlower_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		for (n in 1:NPred) {
			...
			X_TGWPred[n, 2] = tenPodArea_pred[n]; //parents ordered alphabetically
			X_TGWPred[n, 3] = totSeedNum_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_tenPodCompactnessPred[n, 1] = totCompactness_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_totSeedWPred[n, 1] = numPods2ary_pred[n]; //parents ordered alphabetically
			X_totSeedWPred[n, 2] = numPodsMain_pred[n]; //parents ordered alphabetically
			X_totSeedWPred[n, 3] = tenPodWeight_pred[n]; //parents ordered alphabetically
			X_totSeedWPred[n, 4] = TGW_pred[n]; //parents ordered alphabetically
			X_totSeedWPred[n, 5] = totSeedArea_pred[n]; //parents ordered alphabetically
			X_totSeedWPred[n, 6] = totSeedNum_pred[n]; //parents ordered alphabetically
		}
...

	{
		...
		// fill in X array
		for (n in 1:NPred) {
			X_oilContentPred[n, 1] = seedACov_pred[n]; //parents ordered alphabetically
